...
(function () {
  ...
  const CONFIG = {
    ...
    messages: {
      ...
      planImpulso:
        'Hola 👋. Estoy interesado/a en el *Plan Impulso Inicial* ($180.000 COP). ¿Me ayudan con los pasos para empezar?',
      planElite:
        'Hola 👋. Quiero adquirir el *Plan Elite Profesional* ($280.000 COP). ¿Me indican cómo realizar el pago?',
      finalCta:
      ...
      planArquitectura:
        'Hola, Luca. Quiero información sobre el Plan Arquitectura Total, que incluye el Plan Elite más la Sesión de Claridad Profesional.',
    },
  };
  ...
  const track = {
    pageView: function () { dispatch('PageView'); },
    viewContent: function (section) { dispatch('ViewContent', { section: section }); },
    clickWhatsApp: function (source) { dispatch('ClickWhatsApp', { source: source }); },
    selectPlanImpulso: function () { dispatch('SelectPlanImpulso'); },
    selectPlanElite: function () { dispatch('SelectPlanElite'); },
    selectCoachingSession: function () { dispatch('SelectCoachingSession'); },
    selectPlanArquitectura: function () { dispatch('SelectPlanArquitectura'); },
  };
  ...
  function wireWhatsAppLinks() {
    ...
    links.forEach(function (el) {
      ...
      el.addEventListener('click', function () {
        ...
        if (el.classList.contains('wa-plan-impulso')) track.selectPlanImpulso();
        if (el.classList.contains('wa-plan-elite')) track.selectPlanElite();
        if (el.classList.contains('wa-coaching-session')) track.selectCoachingSession();
        if (el.classList.contains('wa-plan-arquitectura')) track.selectPlanArquitectura();
      });
    });
  }
  ...
  function wireFloatingCta() {
    ...
    const targetSection = document.getElementById('planes');
